...
static int print_directory(char *path, const char *name, int state)
{
	...
	if ((stat = malloc(sizeof(*stat))) == NULL)
		return free(buf), DE_MEMORY;

	strcat(path, name);

	if ((r = vfs_open(path, &node, 0, 0)) != 0) {
		r = (r != DE_MEDIA_CHANGED) ? r : 0;
		return free(stat), free(buf), r;
	}

	node->n_stat(node, stat);
	epoch_convert((unsigned long long)stat->write_time.tv_sec, &buf[0]);
	buf[10] = ' ', buf[19] = '\0';

	kernel->print("%s %10llu %s\n",
		&buf[0], (unsigned long long)stat->size, path);

	if (state)
		strcat(path, "/");

	if (node->type == vfs_type_directory) {
		for (i = 0; !r && i < INT_MAX; i++) {
			r = node->n_readdir(node, (uint64_t)i, 128, &buf[0]);

			if (r == DE_PLACEHOLDER)
				continue;

			if (r == DE_OVERFLOW || r == DE_MEDIA_CHANGED) {
				r = 0;
				break;
			}
			...
			if (!r)
				r = print_directory(path, &buf[0], state + 1);
		}
	}
...
int vfs_print_root(void)
{
	...
	if ((path = calloc(1024, sizeof(char))) == NULL)
		return DE_MEMORY;

	kernel->print("\033cDirectory Listing\n\n");
	r = print_directory(path, "/", 0);
	kernel->print("\n\tStatus: %s\n", strerror(r));
